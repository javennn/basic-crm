...
if (isset($_POST['update-payment'])) {
    $id = validate($_POST['id']);  // Get the customer ID
    $paymentAmount = validate($_POST['cust-payment']);  // Get the payment amount

    if (is_numeric($paymentAmount) && $paymentAmount > 0) {
        // Update payment details in the database
        $sqlUpdatePayment = "UPDATE `client` SET `cust_payment` = '$paymentAmount' WHERE `id` = '$id'";
        $resultUpdatePayment = mysqli_query($conn, $sqlUpdatePayment);

        if ($resultUpdatePayment) {
            // Update customer status to 'paid'
            $sqlUpdateStatus = "UPDATE `client` SET `cust_status` = 'paid customer' WHERE `id` = '$id'";
            $resultUpdateStatus = mysqli_query($conn, $sqlUpdateStatus);

            if ($resultUpdateStatus) {
                // Success message
                redirect('sidebar.php?page=clients', 'Success', 'Payment received and status updated.');
            } else {
                ...
                <div class="modal-body" style="max-height: 35em; display: flex; gap: 1.5em; margin: 0em 0em 1em 0em;">
                    <div style="width: 60%;">
                        ...
                        ">
                            ...
                            <input type="text" name="cust-status" id="resolvedTask" class="form-inputs-ro" readonly value="">
                        </div>
                    ...
                    <div style="width: 40%;">
                        <form method="POST">
                            ...
                            <label class="form-labels">Payment</label>
                            <input type="text" name="cust-payment" id="custPayment" class="form-inputs" value=""> 

                            <button type="submit" name="update-payment" class="add-btn" style="margin-top: 1em;">Submit Payment</button>
                        </form>
                    </div>
                </div> 
            </div>
